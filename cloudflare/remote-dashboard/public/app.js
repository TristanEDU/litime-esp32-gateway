import { RuntimeEstimator, formatRuntime } from './runtime-estimate.mjs';
...
(() => {
  ...
  const runtime = new RuntimeEstimator();

  function showRuntime() {
    const estimate = formatRuntime(runtime.snapshot(performance.now()));
    $('runtime-label').textContent = estimate.label;
    $('runtime-value').textContent = estimate.value;
    $('runtime-detail').textContent = estimate.detail;
  }
  ...
  function showBattery(battery) {
    runtime.update(battery, performance.now());
    showRuntime();
