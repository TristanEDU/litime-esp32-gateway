...
With ESP32 Arduino core 3.3.11, a configured remote-dashboard WebSocket build uses **1,501,961 bytes** for release and **1,508,105 bytes** for development in the **2,031,616-byte** OTA slot, leaving roughly **530 KiB** of code-space margin. The NimBLE migration also leaves **263,956 bytes** of reported dynamic-memory headroom at link time for the release build. The 64 KiB LittleFS image is built and flashed at `0x3F0000`; it is intentionally compact, so treat it as a bounded UI/history buffer rather than durable storage. Hardware validation was reported passing on 2026-09-21 for real LiTime BLE plus TLS/WebSocket operation, induced internet-outage recovery, and runtime heap margin with the remote dashboard connected.
